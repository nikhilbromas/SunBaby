# Deploy SunBaby Backend to Windows Server 2025 EC2
...
param(
    ...
    [string]$Dockerfile = "Dockerfile.windows2025"
)

Write-Host "=== SunBaby Backend Deployment Script (Windows Server 2025) ===" -ForegroundColor Green
Write-Host "Building and running container on Windows Server 2025 EC2`n" -ForegroundColor Cyan
...
if (-not (Test-Path $Dockerfile)) {
    ...
    Write-Host "Available Dockerfiles:" -ForegroundColor Yellow
    Get-ChildItem -Filter "Dockerfile.windows*" | ForEach-Object { Write-Host "  - $($_.Name)" -ForegroundColor Cyan }
    exit 1
}
